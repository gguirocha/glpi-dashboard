...
export function TechnicianRankingList({ startDate, endDate }: TechnicianRankingProps) {
    ...
    if (loading) {
        return (
            <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl p-6 h-full flex items-center justify-center shadow-sm">
    ...
    return (
        <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl p-6 h-full flex flex-col shadow-sm transition-colors">
            <div className="flex items-center justify-between mb-6">
                ...
                <span className="text-xs font-medium text-slate-500 dark:text-slate-400 px-2 py-1 bg-slate-100 dark:bg-slate-700 rounded-full">
                    Top Performance
                </span>
            </div>
                    ...
                    ranking.map((tech, index) => (
                        <div
                            ...
                            className={`
                                ...
                                ${index === 0 ? 'bg-amber-50 dark:bg-amber-900/20 border-amber-200 dark:border-amber-700/50' : ''}
                                ${index === 1 ? 'bg-slate-50 dark:bg-slate-700/50 border-slate-200 dark:border-slate-600' : ''}
                                ${index === 2 ? 'bg-orange-50 dark:bg-orange-900/20 border-orange-200 dark:border-orange-700/50' : ''}
                                ${index > 2 ? 'bg-white dark:bg-slate-800 border-slate-100 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700/50' : ''}
                            `}
                        >
                            {/* Rank Badge */}
                            ...
                            <div className="flex items-center justify-between pl-3 gap-2">
                                <div className="min-w-0 flex-1">
                                    <div className="flex items-center gap-2">
                                        <h4 className={`font-semibold truncate ${index <= 2 ? 'text-slate-900 dark:text-white' : 'text-slate-700 dark:text-slate-300'}`}>
                                            {tech.technician_name}
                                        </h4>
                                        {/* Info Tooltip */}
                                        <div className="relative group/info shrink-0">
                                            <Info className="h-4 w-4 text-slate-400 hover:text-indigo-600 dark:hover:text-indigo-400 cursor-help" />
                                            <div className={`absolute left-1/2 -translate-x-1/2 w-56 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 p-3 rounded-lg shadow-xl text-xs z-50 hidden group-hover/info:block text-slate-700 dark:text-slate-300 ${index < 2 ? 'top-full mt-2' : 'bottom-full mb-2'}`}>
